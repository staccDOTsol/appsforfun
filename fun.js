...
if (i == 0 || (segments.length == 1) ){
        return infer(prompt, {"inputs": data.inputs + segments[segments.length-1].toString()}, i+1, segments[segments.length-1].toString())
}else{
    return segments[segments.length-2].toString().replace('\n', ', ') +  segments[segments.length-1].toString();
}
  } catch (err){
    console.log(err)
    return oldresp
...
let text = await infer(prompt.replace('\n', ', '), {"inputs": prompt.replace('\n', ', ')}, 0, "")
